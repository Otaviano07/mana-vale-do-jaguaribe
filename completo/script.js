        ...
        generatePDF() {
            return new Promise(resolve => {
                ...
                img.onload = () => {
                    ...
                    let currentY = 90; // Posição Y inicial para a lista de produtos
                    doc.setFontSize(12);
                    doc.setTextColor(0, 0, 0);
                    doc.text('PRODUTOS SELECIONADOS:', 20, currentY);
                    currentY += 7; // Espaço após o título

                    doc.setFontSize(10);
                    this.products.forEach(product => {
                        if (product.quantity > 0) {
                            const subtotal = (product.price * product.quantity).toFixed(2).replace('.', ',');
                            const productLine = `${product.name} (${product.code}) - ${product.quantity} x R$ ${product.price.toFixed(2).replace('.', ',')} = R$ ${subtotal}`;
                            doc.text(productLine, 25, currentY);
                            currentY += 5; // Espaço entre os produtos
                        }
                    });

                    currentY += 5; // Espaço antes do total

                    // Total
                    doc.setFontSize(12);
                    doc.setFont("helvetica", "bold");
                    doc.text(this.calculateTotal().toFixed(2).replace('.', ','), 130, 320);
                    currentY += 7;

                    // Forma de Pagamento
                    doc.setFontSize(10);
                    doc.setFont("helvetica", "normal");
                    doc.text(`FORMA DE PAGAMENTO: ${this.getPaymentMethodLabel()}`, 20, currentY);
